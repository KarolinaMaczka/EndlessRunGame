...
from difficulty.maps.color_themes import COLOR_THEME_COLORFULL, COLOR_THEME_DARK, COLOR_THEME_BASIC
...
class Difficulty1(Difficulty):
    def __init__(self, first_obstacle=INITIAL_FIRST_OBSTACLE_Z_POS, last_obstacle_z=INITIAL_LAST_OBSTACLE_Z_POS,
        ...
        self.maps = [
            # FirstObstacleMap(lane_change_const=0.1, small_obstacle_const=0.2, gate_generation_const=0.2),
                     FirstObstacleMap(lane_change_const=0.25, small_obstacle_const=0.3, gate_generation_const=0.2, color_theme=COLOR_THEME_BASIC),
    ...
    def generate_obstacle(self, player_z: float) -> list[ObstacleMetaData]:
        if player_z > self.first_obstacle + self.obstacle_generation_distance:
            ...
            self.last_obstacle_z = mapp.generate_obstacles(self.obstacle_generation_distance, self.last_obstacle_z + 200, 2000)
